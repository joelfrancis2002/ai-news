    ...
    <div className="flex h-full flex-col">
      <div className="border-b border-slate-800 bg-slate-950/80 px-6 py-5">
        ...
        <p className="mt-1 text-sm text-slate-500">
          {isReader ? "List of active RSS feeds and crawler sources." : "Manage RSS feeds and source health for the ingestion pipeline."}
        </p>
      </div>
            ...
            <div className="divide-y divide-slate-800/70">
              {data?.data.map((source) => (
                <div key={source.id} className="flex flex-wrap items-center gap-4 px-5 py-4">
                  ...
                  {isReader ? (
                    <span className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${source.enabled ? "bg-emerald-500/10 text-emerald-400" : "bg-slate-800 text-slate-400"}`}>
